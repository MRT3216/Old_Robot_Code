/* This appears to be the code we'll be taking to competition.  There's
 * a few issues, but I intend to fix most of them.  Firstly, misfires.
 * misfires that change the state of the limit switch, no matter how
 * slightly, cause the reset cylinders to actuate.   There's two things
 * (non-code) that we can to to fix this.  1) Put elastic tubing between
 * the cylinders and the arm.  This will ensure that they are always
 * aligned with the arm, regardless of if it's shaking or firing or
 * whatever.  It won't prevent misfires, but it will prevent damage to 
 * the robot when it does.  2) Replace the pneumatic release mechanism
 * with an electric solenoid.  There may not always be air in the system,
 * but there will almost always be 12 volts available.  They're also
 * incredibly strong.
 */
